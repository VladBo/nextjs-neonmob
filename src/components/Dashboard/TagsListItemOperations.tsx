...
async function deleteTagsListItem(tagListId: string) {
  const response = await fetch(`/api/tags-lists/${tagListId}`, {
    method: "DELETE",
  });
...
interface TagsListItemOperationsProps {
  tagsList: Pick<TagsList, "id" | "title">;
}

export function TagsListItemOperations({
  tagsList,
}: TagsListItemOperationsProps) {
          ...
          <Alert.Footer>
            ...
            <Alert.Action
              onClick={async (event) => {
                ...
                const deleted = await deleteTagsListItem(tagsList.id);
