...
async function deleteTagItem(tagId: number) {
...
interface TagItemOperationsProps {
  tag: Pick<Tag, "id" | "title">;
}

export function TagItemOperations({ tag }: TagItemOperationsProps) {
      ...
      <DropdownMenu>
        ...
        <DropdownMenu.Portal>
          <DropdownMenu.Content>
            <DropdownMenu.Item>
              <Link href={`/dashboard/tags/${tag.id}`} className="flex w-full">
                Edit
              </Link>
            </DropdownMenu.Item>
          ...
          <Alert.Footer>
            ...
            <Alert.Action
              onClick={async (event) => {
                ...
                const deleted = await deleteTagItem(tag.id);
